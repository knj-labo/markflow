...
type WasmBindings = typeof import("../wasm/rsmd_core.js");
...
async function loadWasm(): Promise<void> {
  try {
    const wasmModule = (await import("../wasm/rsmd_core.js")) as WasmBindings;

    if (
      typeof wasmModule.render_wasm !== "function" ||
      typeof wasmModule.slugify_wasm !== "function"
    ) {
      throw new Error("Invalid WASM exports");
    }

    wasmRuntime.storeModule({
      render_wasm: (source: string, options: WasmOptions) =>
        wasmModule.render_wasm(source, options) as RenderResult,
      slugify_wasm: (text: string) => wasmModule.slugify_wasm(text),
    });
  } catch (error) {
